...
### 改进
- 下载运行命令会自动判断当前用户是否为 root：root 用户直接运行脚本，非 root 用户优先使用 `sudo`，无 `sudo` 时回退到脚本自身权限提示。
- 非 root 运行脚本时给出更明确的权限提示，并说明无 `sudo` 环境可切换到 root 后执行。

### 修复
- 调整 IPv4 校验中的 Bash 正则和数组解析写法，提升脚本兼容性。
